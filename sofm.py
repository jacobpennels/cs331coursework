...
if(gui_available):
	...
	def show_points():
		...
		pygame.draw.circle(window, (0, 255, 0), (int(screen_size * u[int((len(u)-1)/2)][0]), int(screen_size * u[int((len(u)-1)/2)][1])), 20)
		pygame.draw.circle(window, (0, 0, 255), (int(screen_size * u[len(u) - 1][0]), int(screen_size * u[len(u) - 1][1])), 20)
...
for ex in experiments:
	for b_val in b_vals:
		print("Experimenting for " + str(ex) + "x" + str(ex) + " grid, b value " + str(b_val))
		dimensions = ex
		square = dimensions**2
		random.seed(331) # use same seed every time 
		...
		for i in range(dimensions):
			for j in range(dimensions):
				convergent_points.append((step * i, 1.0 - (step * j)))

		#print(convergent_points)

		data = open("data_" + str(ex) + "_" + str(b_val) + ".txt", "w")
